...
System_Lib() {
    if [ "${PM}" == "yum" ] || [ "${PM}" == "dnf" ]; then
        ...
        ${PM} install ${Pack} -y
        yum install zlib-devel chkconfig -y
...
Install_LuaJIT() {
    ...
    OPEN_LUAJIT=$(cat /usr/local/include/luajit-2.1/luajit.h|grep 2022)
    if [ ! -f '/usr/local/lib/libluajit-5.1.so' ] || [ ! -f "/usr/local/include/${LUAJIT_INC_PATH}/luajit.h" ] || [ "${OPEN_LUAJIT}" ]; then
...
Download_Src() {
    ...
    if [ -z "${GMSSL}" ]; then
        ...
        if [ "${version}" == "tengine" ] || [ "${version}" == "openresty" ] || [ "${version}" == "1.22" ] || [ "${version}" == "1.23" ] || [ "${version}" == "1.24" ] || [ "${version}" == "1.25" ] || [ "${version}" == "1.26" ]; then
            opensslVer="3.2.0"
        fi
...
Install_Nginx() {
    ...
    if [ "${version}" == "openresty" ]; then
        ln -sf ${Setup_Path}/nginx/html ${Setup_Path}/html
        ln -sf ${Setup_Path}/nginx/conf ${Setup_Path}/conf
        ln -sf ${Setup_Path}/nginx/logs ${Setup_Path}/logs
        ln -sf ${Setup_Path}/nginx/sbin ${Setup_Path}/sbin
        if [ -d "/www/server/btwaf" ]; then
            cp -rpa ${Setup_Path}/lualib/* /www/server/btwaf
        elif [ -d "/www/server/free_waf" ];then
            cp -rpa ${Setup_Path}/lualib/* /www/server/free_waf
        fi
    fi
    ...
    if [ ! -f "${Setup_Path}/sbin/nginx" ]; then
        ...
        if [ ! -f "/www/server/panel/install/nginx_down.pl" ];then
            FILE_KEY=("./configure: error: no ${Setup_Path}/src/ngx_devel_kit/config was found" "./configure: No such file or directory" "./configure: error: no ${Setup_Path}/src/ngx_http_substitutions_filter_module-master/config was found" "./configure: error: no ${Setup_Path}/src/nginx-dav-ext-module/config was found" "auto/options: No such file or directory" "./configure: error: no ${Setup_Path}/src/ngx_cache_purge/config was found" "${Setup_Path}/src/lua_nginx_module/config was found")
    ...
    if [ "${version}" == "1.22" ] || [ "${version}" == "1.23" ] || [ "${version}" == "1.24" ] || [ "${version}" == "tengine" ] || [ "${version}" == "1.25" ] || [ "${version}" == "1.26" ];then
...
Set_Conf() {
    ...
    chmod 755 ${Setup_Path}/html/
    chmod 755 /www/wwwroot/
    chmod 644 ${Setup_Path}/html/*
...
EOF
    echo "" >${Setup_Path}/conf/enable-php-00.conf
...
lua_package_path "${Setup_Path}/waf/?.lua";
init_by_lua_file  ${Setup_Path}/waf/init.lua;
access_by_lua_file ${Setup_Path}/waf/waf.lua;
EOF
    ...
    if [ "${AA_PANEL_CHECK}" ]; then
        #cp -rf /www/server/panel/data/empty.html ${Setup_Path}/html/index.html
        wget -O ${Setup_Path}/html/index.html ${download_Url}/error/index_en_nginx.html -T 20
        chmod 644 ${Setup_Path}/html/index.html
    ...
    if [ "${version}" == "1.22" ] || [ "${version}" == "1.23" ] || [ "${version}" == "1.24" ] || [ "${version}" == "tengine" ] || [ "${version}" == "1.25" ] || [ "${version}" == "1.26" ];then
        # if [[ -z $ARM_CHECK ]]; then
            ...
            if [ -d "/www/server/btwaf" ];then
                ...
                cp -rpa ${Setup_Path}/lib/lua/* /www/server/btwaf
            elif [ -d "/www/server/free_waf" ];then
                rm -rf /www/server/btwaf/ngx
                rm -rf /www/server/btwaf/resty
                cp -rpa ${Setup_Path}/lib/lua/* /www/server/free_waf
            else
                sed -i '/lua_package_path/d' ${Setup_Path}/conf/nginx.conf
                sed -i '/include proxy\.conf;/a \        lua_package_path "${Setup_Path}/lib/lua/?.lua;;";' ${Setup_Path}/conf/nginx.conf
            fi
        # else
        #     echo "Disabling Lua . . ."
        #     sed -i '/lua_package_path/d' ${Setup_Path}/conf/nginx.conf
        ...
        # the init script has Lua checker, so we need to disable it . . .
        # sed -i 's/\$(cat \/www\/server\/nginx\/conf\/nginx.conf|grep "nginx\/lib\/lua")/false/' /etc/init.d/nginx
    fi
    if [ "${version}" == "1.25" ] || [ "${version}" == "1.26" ];then
        HTTP_POST_CHECK=$(cat ${Setup_Path}/conf/fastcgi.conf|grep "HTTP_HOST")
        if [ -z "${HTTP_POST_CHECK}" ];then
            echo "fastcgi_param  HTTP_HOST          \$host;" >> ${Setup_Path}/conf/fastcgi.conf
        fi
    fi
...
if [ "${actionType}" == "uninstall" ]; then
    ...
    *)
    ...
    # Setup_Path=$Setup_Path/$nginxVersion
    ...
    if [ "${actionType}" == "install" ]; then
        ...
        echo "Instaling nginx on $Setup_Path . . ."
        ...
        echo "Updating nginx on $Setup_Path to version $nginxVersion. . ."
